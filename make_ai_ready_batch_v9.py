...
INVENTORY_SHIPS_VARS = ['incv_kt', 'dtl_km', 'shrd_kt', 'shtd_deg', 'rhmd_pct', 'vmpi_kt']
...
CREATOR_EMAIL   = 'kathryn.sellwood@noaa.gov, altug.aksoy@noaa.gov'
CREATOR_NAME    = 'Kathryn Sellwood, Altug Aksoy'
TITLE           = 'hrdobs consolidated tc reconnaissance observations'
VERSION_NUMBER  = 'v1.0'
...
def convert_universal(input_h5, output_h5, scan_only=False, spline_alt_mb=None,
    ...
    try:
        ...
        with h5py.File(input_h5, 'r') as f_in, out_ctx as f_out:
            ...
            # ----------------------------------------------------------------
            # Helper: write global attribute (and track in schema)
            # ----------------------------------------------------------------
            ...
            EXCLUDED_GLOBAL_ATTRS = {
                'manifest expected', 'original source count', 'validation status'
            }
            ...
            def set_meta(k, v):
                ...
                if not scan_only:
                    # Decode bytes/arrays fully before casting to str so that
                    # raw HDF5 byte-arrays (e.g. array([b'kathryn...'])) never
                    # produce the literal "b'" prefix in the output attribute.
                    f_out.attrs[k] = str(decode_attr(v))
            ...
            if not scan_only:
                if 'center_from_tc_vitals' not in f_out.attrs:
                    set_meta('center_from_tc_vitals', 'NaN')
                if 'tc_category' not in f_out.attrs:
            ...
            if clat_val is not None and clon_val is not None:
                ...
                if not scan_only and 'center_from_tc_vitals' not in f_out.attrs:
                    set_meta('center_from_tc_vitals', 'NaN')
            ...
            if not scan_only:
                ...
                for crit_key in sorted(CRITICAL_METADATA):
                    crit_val = str(f_out.attrs.get(crit_key, 'NaN')).strip()
            ...
            def process_obs_leaf(group_name, sub_grp, parent_missing=None):
                ...
                # --- Build error fields ---
                skip_err_for = COORD_VARS | TRACK_PRODUCT_VARS
                for dset_name in list(df_dict.keys()):
                    if dset_name.lower() in skip_err_for:
                        continue
                    err_name = f"{dset_name}err"
                    if err_name in df_dict:
                        continue
                    err_val  = None
                    raw_attr = sub_grp[dset_name].attrs.get(
                        'error_estimate_for_data_assimilation'
                    ) if dset_name in sub_grp else None
                    if raw_attr is not None:
                        err_val = extract_error_val(raw_attr)
                    if err_val is None and dset_name.lower() in ERROR_FALLBACK_RULES:
                        err_val = ERROR_FALLBACK_RULES[dset_name.lower()]
                    if err_val is not None:
                        err_dict[err_name] = {'base_var': dset_name, 'val': err_val}

                # --- Assemble DataFrame + inject constant errors ---
                df = pd.DataFrame(df_dict)
                for err_name, info in err_dict.items():
                    df[err_name] = float(info['val'])
                ...
                #
                # Drop rows whose lat/lon falls outside the OBS_PROXIMITY_DEG
                # box around the storm center at cycle time.  Only applied
                # when a best-track center fix was successfully resolved
                # (clat_val / clon_val are not None); if the center lookup
                # failed the filter is silently skipped so a metadata gap
                # never causes all observations to be discarded.
                # ---------------------------------------------------------
                ...
                if not scan_only:
                    ...
                    for col in df_clean.columns:
                        ...
                        elif col in err_dict:
                            base_var = err_dict[col]['base_var']
                            dset_out.attrs['long_name'] = (
                                f"error estimate for {base_var}"
                            )
            ...
            # ----------------------------------------------------------------
            # 4b. Write controlled metadata attributes
            # ----------------------------------------------------------------
            if not scan_only:

                # ── Constants ──────────────────────────────────────────
                f_out.attrs['creator_email']  = CREATOR_EMAIL
                f_out.attrs['creator_name']   = CREATOR_NAME
                f_out.attrs['title']          = TITLE
                f_out.attrs['version_number'] = VERSION_NUMBER
                for ck in ('creator_email', 'creator_name',
                           'title', 'version_number'):
                    schema_updates['global'].add(ck)

                # ── Platforms (derived from actual output groups) ──────
                platforms_str = ', '.join(sorted(obs_groups))
                f_out.attrs['platforms'] = platforms_str
                schema_updates['global'].add('platforms')

                # ── Storm motion (from source, or NaN) ────────────────
                f_out.attrs['storm_motion'] = source_storm_motion
                schema_updates['global'].add('storm_motion')
                ...
                if fn_match:
                    ...
                    f_out.attrs['time_coverage_start'] = \
                        tc_start.strftime("%Y-%m-%dT%H:%M:%SZ")
                    f_out.attrs['time_coverage_end'] = \
                        tc_end.strftime("%Y-%m-%dT%H:%M:%SZ")
                schema_updates['global'].add('time_coverage_start')
                schema_updates['global'].add('time_coverage_end')
                ...
                if clat_val is not None and clon_val is not None:
                    ...
                    f_out.attrs['geospatial_lat_min'] = f"{geo_lat_min:.2f}"
                    f_out.attrs['geospatial_lat_max'] = f"{geo_lat_max:.2f}"
                    f_out.attrs['geospatial_lon_min'] = f"{geo_lon_min:.2f}"
                    f_out.attrs['geospatial_lon_max'] = f"{geo_lon_max:.2f}"
                else:
                    # No center available — write NaN
                    f_out.attrs['geospatial_lat_min'] = 'NaN'
                    f_out.attrs['geospatial_lat_max'] = 'NaN'
                    f_out.attrs['geospatial_lon_min'] = 'NaN'
                    f_out.attrs['geospatial_lon_max'] = 'NaN'

                f_out.attrs['geospatial_lat_units'] = 'degrees north'
                f_out.attrs['geospatial_lon_units'] = 'degrees east'
                for gk in ('geospatial_lat_min', 'geospatial_lat_max',
                           'geospatial_lat_units',
                           'geospatial_lon_min', 'geospatial_lon_max',
                           'geospatial_lon_units'):
                    schema_updates['global'].add(gk)
                ...
                with h5py.File(output_h5, 'r') as f_check:
                    for field in sorted(CRITICAL_METADATA):
                        if field not in f_check.attrs:
                            ...
                            val = decode_attr(f_check.attrs[field]).strip()
                            if val in ('', 'NaN', 'nan'):
                                meta_issues.append(
...
def extract_inventory_and_schema(h5_path):
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        def get_attr(key, default="NaN"):
            ...
            return decode_attr(val) if val != default else default
...
def main():
    ...
    if mode == '5':
        ...
        # Load SHIPS lookup so mismatch advisories appear in QC scan output
        ...
        simulated_written = 0   # Track written files
        skip_reason_counts = {} # Track the frequency of each skip reason
        ...
        for i, input_path in enumerate(input_files):
            ...
            if qc:
                ...
                # Look for skip reasons exactly like Mode 6 does
                skip_reasons = [
    ...
    if mode == '6':
        ...
        # ── Load SHIPS lookup ─────────────────────────────────────────────
        ships_lookup = load_ships_lookup(SHIPS_CSV_PATH)
        ships_matched = 0   # files where a SHIPS cycle was found and written
        ...
        for i, input_path in enumerate(input_files):
            ...
            # Simpler: count files that have ships_params in their schema
        ...
        if ships_lookup:
            ships_written = sum(
                ...
                if log.get('Group') == 'ships_params'
                and log.get('Variable') == 'MATCH'   # no-match advisory rows
            )
            n_no_match = ships_written   # these are the no-match advisories
            ...
            # ── SHIPS metadata mismatch report ─────────────────────────────
            _mismatch_csv = "ships_metadata_mismatches.csv"
            if all_ships_mismatches:
                pd.DataFrame(all_ships_mismatches).to_csv(_mismatch_csv, index=False)
